...
const takeRandomQuiz = (quizes, output, n, getGrade) => Promise.all(quizes.map(fileName => (0, _lib.readFile)(fileName))).then(values => [].concat.apply([], values)).then(combineTest => (0, _lib.chooseRandom)(combineTest, n)).then(randomTest => (0, _inquirer.prompt)(randomTest).then(answers => (0, _lib.writeFile)(output, answers))).then(info => {
  if (getGrade) return gradeTakenQuiz(output, quizes, getGrade);
}).catch(err => console.log('Error while taking random Quiz.', err));

//Grade taken test
const gradeTakenQuiz = (takenQuiz, quizes) => {
  Promise.all(quizes.map(fileName => (0, _lib.readFile)(fileName + '-answers'))).then(values => [].concat.apply([], values)).then(answers => (0, _lib.readFileExtra)(takenQuiz, answers)).then(info => (0, _lib.gradeQuiz)(info[0], info[1])).catch(err => console.log('Error while grading Quiz.', err));
};
