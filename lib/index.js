...
//Create a quiz
const createQuiz = title => (0, _inquirer.prompt)(askForQuestions).then(answer => (0, _lib.createPrompt)(answer)).then(createdPrompt => (0, _inquirer.prompt)(createdPrompt).then(inputedQuestions => (0, _lib.createQuestions)(inputedQuestions)).then(questions => JSON.stringify(questions, null, 2)).then(info => (0, _lib.writeFile)(title, info))).catch(err => console.log('Error creating the quiz.', err));

//Take a quiz and put answeres in outputfile
const takeQuiz = (title, output) => (0, _lib.readFile)(title).then(data => JSON.parse(data)).then(quiz => (0, _inquirer.prompt)(quiz).then(answers => JSON.stringify(answers, null, 2)).then(info => (0, _lib.writeFile)(output, info))).catch(err => console.log('Error while taking Quiz.', err));
...
const combineQuizes = quizes => {
  Promise.map(quizes, fileName => (0, _lib.readFile)(fileName));
};

//Fuse all the quizes together and return a random quiz from all,
// then put answers into output
const takeRandomQuiz = (quizes, output, n) => combineQuizes(quizes).then(combineTest => console.log(combineTest)).catch(err => console.log('Error while taking random Quiz.', err));
