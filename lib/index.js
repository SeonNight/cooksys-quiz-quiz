...
const takeQuiz = (title, output, getGrade) => (0, _lib.readFile)(title).then(quiz => (0, _inquirer.prompt)(quiz).then(answers => (0, _lib.writeFile)(output, answers))).then(info => {
  if (getGrade) return gradeTakenQuiz(output, [title]);
...
const takeRandomQuiz = (quizes, output, n, getGrade) => Promise.all(quizes.map(fileName => (0, _lib.readFile)(fileName))).then(values => [].concat.apply([], values)).then(combineTest => (0, _lib.chooseRandom)(combineTest, n)).then(randomTest => (0, _inquirer.prompt)(randomTest).then(answers => (0, _lib.writeFile)(output, answers))).then(info => {
  if (getGrade) return gradeTakenQuiz(output, quizes);
}).catch(err => console.log('Error while taking random Quiz.', err));

//Grade taken test
const gradeTakenQuiz = (takenQuiz, quizes) => Promise.all(quizes.map(fileName => (0, _lib.readFile)(fileName + '-answers'))).then(values => [].concat.apply([], values)).then(answers => (0, _lib.readFileExtra)(takenQuiz, answers)).then(info => (0, _lib.gradeQuiz)(info[0], info[1])).catch(err => console.log('Error while grading Quiz.', err));
...
cli.command('take <fileName> <outputFile>', 'Loads a quiz and saves the users answers to the given outputFile').option('-g, --grade', 'Grade quiz after completion').action(function (input, callback) {
  return takeQuiz(input.fileName, input.outputFile, input.options.grade);
});

cli.command('random <outputFile> <fileNames...>', 'Loads a quiz or' + ' multiple quizes and selects a random number of questions from each quiz.' + ' Then, saves the users answers to the given outputFile').option('-g, --grade', 'Grade quiz after completion').option('-q, --questions <number>', 'Input the number of questions you want in the quiz').action(function (input, callback) {
